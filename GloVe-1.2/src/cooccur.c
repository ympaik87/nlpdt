...
int get_cooccurrence()
{
    ...
    while (1)
    {
        ...
        {                                  // Iterate over all words to the left of target word, but not past beginning of line
            ...
            { // Product is too big, data is likely to be sparse. Store these entries in a temporary buffer to be sorted, merged (accumulated), and written to file when it gets full.
                cr[ind].word1 = w1;
                cr[ind].word2 = w2;
